...
public class SpotAnim
{
    ...
    public static void initialize(byte byte0, Class10 class10)
    {
        ...
        anInt187 = class39_sub3_sub3.getShort();
    ...
    public void method195(int i, Buffer class39_sub3_sub3)
    {
        ...
        do
        {
            ...
            if(j == 0)
                return;
            if(j == 1)
                anInt190 = class39_sub3_sub3.getShort();
            else
            if(j == 2)
            {
                anInt191 = class39_sub3_sub3.getShort();
                if(SEQ.aClass20Array271 != null)
                    aClass20_192 = SEQ.aClass20Array271[anInt191];
            } else
            if(j == 4)
                anInt195 = class39_sub3_sub3.getShort();
            else
            if(j == 5)
                anInt196 = class39_sub3_sub3.getShort();
            else
            if(j == 6)
                anInt197 = class39_sub3_sub3.getShort();
            else
            if(j == 7)
                anInt198 = class39_sub3_sub3.getUnsignedByte();
            else
            if(j == 8)
                anInt199 = class39_sub3_sub3.getUnsignedByte();
            else
            if(j >= 40 && j < 50)
                anIntArray193[j - 40] = class39_sub3_sub3.getShort();
            else
            if(j >= 50 && j < 60)
                anIntArray194[j - 50] = class39_sub3_sub3.getShort();
            else
